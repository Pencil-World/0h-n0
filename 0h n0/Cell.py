...
class Cell:
    ...
    def printActive(self):
        ...
        print(f"horzLim: {self.limits[0]}, vertLim: {self.limits[2]}")
    ...
    # source cell may be out of range of the target cell because it connects to a cluster which creates overstepping for the target cell
    def update(self):
    ...
    def predict(self):
        ...
        while repeat:
            ...
            for i, temp in enumerate([self.right, self.left, self.bottom, self.top]):
                ...
                for count, elem in enumerate(arr):
                    ...
                    if elem.state < 0 and count > len(arr) - diff:
                        break
                    repeat = repeat or elem.markBlue()
            ...
            if sum([elem.state >= 0 for elem in self.left[self.leftVal + 1:self.freedom + self.leftVal + 1]]) == self.freedom:
                repeat = repeat or self.left[self.leftVal].markRed()
